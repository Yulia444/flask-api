...
def api_post_event(eventid):
    if request.method == "POST":
        enrollment = Enrollments(event_id=eventid,
...
def api_post_register():
    ...
    if Participants.query.filter_by(email=email).first():
        return jsonify({"status": "error"})
    participant = Participants(name=name, email=email, location=location,
                               about=about)
    participant.set_password(password)
    db.session.add(participant)
    db.session.commit()
    return jsonify(dict(id=Participants.query.count(), name=name, email=email,
                        password=password, location=location, about=about))
